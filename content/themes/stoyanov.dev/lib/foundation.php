...
if (!function_exists('ps_get_the_comments_pagination')) :
    function ps_get_the_comments_pagination($args = array())
    {
        $navigation = '';
        $args = wp_parse_args($args, array(
            'prev_text'				=> __('&laquo;', 'ps'),
            'next_text'				=> __('&raquo;', 'ps'),
            'size'					=> 'default',
            'show_disabled'			=> true,
        ));
        $args['type'] = 'array';
...
// Custom Comments Pagination.
if (!function_exists('ps_the_comments_pagination')) :
    function ps_the_comments_pagination($args = array())
    {
        echo ps_get_the_comments_pagination($args);
    }
endif;
...
if (!function_exists('ps_menu_fallback')) :
    function ps_menu_fallback()
    {
        echo '<div class="alert-box secondary">';
        /* translators: %1$s: link to menus, %2$s: link to customize. */
        printf(
            __('Please assign a menu to the primary menu location under %1$s or %2$s the design.', 'ps'),
            /* translators: %s: menu url */
            sprintf(
                __('<a href="%s">Menus</a>', 'ps'),
                get_admin_url(get_current_blog_id(), 'nav-menus.php')
            ),
            /* translators: %s: customize url */
            sprintf(
                __('<a href="%s">Customize</a>', 'ps'),
...
if (!function_exists('ps_active_nav_class')) :
    function ps_active_nav_class($classes, $item)
    {
    ...
    add_filter('nav_menu_css_class', 'ps_active_nav_class', 10, 2);
endif;
 ...
 */
if (!function_exists('ps_active_list_pages_class')) :
    function ps_active_list_pages_class($input)
    {
    ...
    add_filter('wp_list_pages', 'ps_active_list_pages_class', 10, 2);
endif;
...
if (!function_exists('ps_mobile_menu_id')) :
    function ps_mobile_menu_id()
    {
...
if (!function_exists('ps_title_bar_responsive_toggle')) :
    function ps_title_bar_responsive_toggle()
    {
 ...
 */
if (!function_exists('ps_gallery')) :
    function ps_gallery($attr)
    {
